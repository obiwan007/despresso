...
// }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(S.of(context).screenShotEditTitle('${DateFormat.yMd().format(_editedShot.date)} ${DateFormat.Hm().format(_editedShot.date)}')),
        actions: <Widget>[
          // Builder(
          ...
          child: Card(
            child: Padding(
              ...
              child: Column(children: [
                KeyValueWidget(width: width, label: S.of(context).recipe, value: _editedShot.recipe.target?.name ?? "No Recipe"),
              ...
              children: [
                  ...
                  child: Column(
                    children: [
                      ReactiveTextField<double>(
                        ...
                        onChanged: (control) =>
                            {currentForm?.controls['extractionYield']?.value = (_editedShot.pourWeight * (control.value ?? 0.0)) / _editedShot.doseWeight},
                      ),
                      ReactiveTextField<double>(
                ...
                if (settingsService.hasRefractometer && refractometerService.state == RefractometerState.connected)
                  Padding(
                    ...
                    child: OutlinedButton(onPressed: () => {getRefractometerData()}, child: const Text('Read TDS from Refractometer')),
                  )
              ],
            ),
          )),
